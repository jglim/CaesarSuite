...
(C) Richard Prinz, License: MIT 
From https://www.codeproject.com/Articles/19274/A-printf-implementation-in-C
License originally indicated in the article, this comment block is separately added by jg
...
namespace CaesarInterpreter
{
	public static class FormatString
	{
		...
		public static string InterpreterSprintf(string Format, Interpreter ih)
		{
			#region Variables
			...
			while (m.Success)
			{
				...
				switch (char.ToLower(formatSpecifier)) 
				{
					case 'c':
						{
							o = (char)ih.Stack.ReadI8();
							break;
						}
					case 'd':
					case 'i':
					...
					case 's': 
						{
							int strPointer = ih.Stack.ReadI32();
							o = InterpreterMemory.GetCStringAtAddress(ih, strPointer);
							break;
						}
				}
